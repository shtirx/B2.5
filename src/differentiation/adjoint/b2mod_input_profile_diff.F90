...
!  Differentiation of source_input in reverse (adjoint) mode (with options context noISIZE r8):
!   gradient     of useful results: smo0 shi0 sna0 *(state_ext.she)
...
!
  SUBROUTINE SOURCE_INPUT_B(mpg, mpgb, geo, geob, switch, switchb, &
&   state_ext, state_extb, ns, sna0, sna0b, smo0, smo0b, she0, she0b, &
&   shi0, shi0b, sch0, sch0b, sne0, sne0b, nc)
!**************************************************************************
...
&   icvend, ireg
    ...
    REAL(kind=r8) :: she0(mpg%ncv, 0:3), shi0(mpg%ncv, 0:3), sch0(mpg%&
&   ncv, 0:3)
    REAL(kind=r8) :: she0b(mpg%ncv, 0:3), shi0b(mpg%ncv, 0:3), sch0b(mpg&
&   %ncv, 0:3)
    REAL(kind=r8) :: sna0(mpg%ncv, 0:1, 0:ns-1)
    REAL(kind=r8) :: sna0b(mpg%ncv, 0:1, 0:ns-1)
    REAL(kind=r8) :: sne0(mpg%ncv, 0:1)
    REAL(kind=r8) :: sne0b(mpg%ncv, 0:1)
    REAL(kind=r8) :: smo0(mpg%ncv, 0:3, 0:ns-1)
    REAL(kind=r8) :: smo0b(mpg%ncv, 0:3, 0:ns-1)
    ...
    REAL(kind=r8), ALLOCATABLE :: fxb(:), xb(:), xfb(:), dxb(:)
...
! initialise sources and data to zero
    IF (nc .EQ. 0) THEN
      CALL PUSHINTEGER4ARRAY(nsdata, 2*4*(nss+1))
      nsdata(:, :, :) = 0
      CALL PUSHINTEGER4ARRAY(nxdata, 2*4*(nss+1))
      nxdata(:, :, :) = 0
...
&           .AND. sources_time_switch .GT. 0.0_R8
    ...
    ELSE
      IF (sources_time_mod .GT. 0.0_R8) THEN
        CALL PUSHCONTROL2B(0)
        catch_up = (MOD(tim, sources_time_mod) .GE. sources_time_switch &
&         .AND. sources_time_switch .GT. 0.0_R8) .OR. INT(tim/&
&         sources_time_mod) .GT. sr_ip_elm_count
      ELSE IF (sources_time_switch .GT. 0.0_R8) THEN
        catch_up = tim .GE. sources_time_switch
        IF (catch_up) THEN
          CALL PUSHCONTROL2B(1)
...
&               0) THEN
            ...
            END IF
          END DO
          ALLOCATE(xfb(0:nxdat+1))
          xfb = 0.D0
          ALLOCATE(xf(0:nxdat+1))
          IF (nxdat .GT. 0) THEN
            ALLOCATE(fxb(1:nxdat))
            fxb = 0.D0
            ALLOCATE(fx(1:nxdat))
            ALLOCATE(xb(1:nxdat))
            xb = 0.D0
...
&                 ireg, 4) .EQ. 2)) THEN
              ...
              END IF
            END DO
            CALL PUSHINTEGER4(ix - 1)
            DEALLOCATE(xr)
            CALL PUSHINTEGER4(chunklength)
            IF (ALLOCATED(xfb)) THEN
              DEALLOCATE(xfb)
            END IF
            DEALLOCATE(xf)
            CALL PUSHINTEGER4(chunklength)
...
!djm Jan2017 Keep linearised sources for balance
    sch0b(:, 0) = sch0b(:, 0) + state_extb%sch
    state_extb%sch = 0.D0
    shi0b(:, 0) = shi0b(:, 0) + state_extb%shi
    state_extb%shi = 0.D0
    she0b(:, 0) = she0b(:, 0) + state_extb%she
    state_extb%she = 0.D0
    smo0b(:, 0, :) = smo0b(:, 0, :) + state_extb%smo
    state_extb%smo = 0.D0
    sna0b(:, 0, :) = sna0b(:, 0, :) + state_extb%sna
    state_extb%sna = 0.D0
    CALL B2XZDD_BWD(mpg%ncv, 3, switch, mpg, sch0, sch0b)
    CALL B2XZDD_BWD(mpg%ncv, 3, switch, mpg, shi0, shi0b)
    CALL B2XZDD_BWD(mpg%ncv, 3, switch, mpg, she0, she0b)
    DO is=ns-1,0,-1
      CALL B2XZDD_BWD(mpg%ncv, 3, switch, mpg, smo0(1, 0, is), smo0b(1, &
&               0, is))
      CALL B2XZDD_BWD(mpg%ncv, 1, switch, mpg, sna0(1, 0, is), sna0b(1, &
&               0, is))
    END DO
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) GOTO 160
 120 CALL POPINTEGER4(ad_count0)
    DO i1=1,ad_count0
      IF (i1 .EQ. 1) THEN
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) GOTO 150
      ELSE
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          GOTO 140
        ELSE
          ...
          DO iy=ad_to0,1,-1
            CALL POPINTEGER4(chunklength)
            ALLOCATE(xf(chunklength))
            ALLOCATE(xfb(chunklength))
            CALL POPINTEGER4(chunklength)
            ALLOCATE(xr(chunklength))
            CALL POPINTEGER4(ad_to)
            DO ix=ad_to,1,-1
              CALL POPCONTROL3B(branch)
              IF (branch .LT. 3) THEN
                IF (branch .EQ. 0) THEN
                  sna0b(icv, 0, spec) = 0.D0
                ELSE IF (branch .EQ. 1) THEN
                  smo0b(icv, 0, spec) = 0.D0
                ELSE
                  she0b(icv, 0) = 0.D0
                END IF
 ...
 130  DEALLOCATE(xf)
      ...
      DO i=nkind_data,1,-1
        CALL POPCONTROL1B(branch)
      END DO
      CALL POPINTEGER4(nxdat)
 140  DO i=nkind_data,1,-1
        CALL POPCONTROL1B(branch)
      END DO
      CALL POPINTEGER4(ndat)
 150  CALL POPINTEGER4(spec)
    END DO
 160 CALL POPCONTROL2B(branch)
    IF (branch .NE. 0) THEN
      IF (branch .NE. 1) GOTO 120
    END IF
    ...
    IF (branch .EQ. 0) THEN
      ...
      CALL POPCONTROL1B(branch)
    ELSE
      IF (branch .EQ. 1) CALL POPBOOLEAN(new_files)
      CALL POPCONTROL2B(branch)
    END IF
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      ...
      CALL POPINTEGER4ARRAY(nxdata, 2*4*(nss+1))
      CALL POPINTEGER4ARRAY(nsdata, 2*4*(nss+1))
      smo0b = 0.D0
      shi0b = 0.D0
      sna0b = 0.D0
      sch0b = 0.D0
...
&       sources_time_mod), sources_time_switch, INT(tim/sources_time_mod&
&       ), sr_ip_elm_count
        catch_up = (MOD(tim, sources_time_mod) .GE. sources_time_switch &
&         .AND. sources_time_switch .GT. 0.0_R8) .OR. INT(tim/&
&         sources_time_mod) .GT. sr_ip_elm_count
...
&                     TRIM(sources_filename)
      ...
      END IF
      IF (catch_up) THEN
        IF (sources_time_mod .GT. 0.0_R8) sr_ip_elm_count = INT(tim/&
&           sources_time_mod)
        CALL READ_SOURCE_PROFILE()
        new_files = .true.
      END IF
...
!-----------------------------------------------------------------------
!
  SUBROUTINE TRANSPORT_INPUT_B(ncv, ns, geo, geob, mpg, pl, plb, dv, dvb&
&   , rt, rtb, hcib, hcibb, dna0, dna0b, dpa0, dpa0b, vla0, vla0b, vsa0&
&   , vsa0b, hci0, hce0, hce0b, sig0, sig0b, alf0, alf0b, ncall)
!
!**************************************************************************
