...
&               (rt%rlqa, 3)/8)
  CALL PUSHREAL8ARRAY(rt%rlrd, r8*SIZE(rt%rlrd, 1)*SIZE(rt%rlrd, 2)*SIZE&
&               (rt%rlrd, 3)/8)
  CALL PUSHREAL8ARRAY(rt%rlbr, r8*SIZE(rt%rlbr, 1)*SIZE(rt%rlbr, 2)*SIZE&
&               (rt%rlbr, 3)/8)
...
&       , is)
      tt = (t1-t0)/(rtlt(ite+1)-rtlt(ite))
      IF (ltst0) THEN
        CALL PUSHREAL8(rt%rlrd(icv, 1, is), r8/8)
        rt%rlrd(icv, 1, is) = TRIMG_FWD(-20.0_R8, 20.0_R8, tt)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHREAL8(rt%rlrd(icv, 1, is), r8/8)
        rt%rlrd(icv, 1, is) = TRIMG_FWD(-20.0_R8, 20.0_R8, tt)
        CALL PUSHCONTROL1B(1)
      END IF
      CALL PUSHREAL8(rt%rlrd(icv, 0, is), r8/8)
...
&       , is)
      tt = (t1-t0)/(rtlt(ite+1)-rtlt(ite))
      IF (ltst0) THEN
        CALL PUSHREAL8(rt%rlbr(icv, 1, is), r8/8)
        rt%rlbr(icv, 1, is) = TRIMG_FWD(-20.0_R8, 20.0_R8, tt)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHREAL8(rt%rlbr(icv, 1, is), r8/8)
        rt%rlbr(icv, 1, is) = TRIMG_FWD(-20.0_R8, 20.0_R8, tt)
        CALL PUSHCONTROL1B(1)
      END IF
      CALL PUSHREAL8(rt%rlbr(icv, 0, is), r8/8)
...
&       t1b + (rtlza(ite, ine+1, is)-rtlza(ite, ine, is))*t0b
      t0 = (1.0_R8-fxne)*rtlbr(ite, ine, is) + fxne*rtlbr(ite, ine+1, is&
&       )
      CALL POPREAL8(rt%rlbr(icv, 0, is), r8/8)
      fxteb = fxteb + (t1-t0)*rtb%rlbr(icv, 0, is)
      t0b = (1.0_R8-fxte)*rtb%rlbr(icv, 0, is)
      t1b = fxte*rtb%rlbr(icv, 0, is)
      rtb%rlbr(icv, 1, is) = rtb%rlbr(icv, 1, is) - rlte*rtb%rlbr(icv, 0&
&       , is)
      rlteb = rlteb - rt%rlbr(icv, 1, is)*rtb%rlbr(icv, 0, is)
      rtb%rlbr(icv, 0, is) = 0.D0
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL TRIMG_BWD(-20.0_R8, 20.0_R8, tt, ttb, rtb%rlbr(icv, 1, is))
        rtb%rlbr(icv, 1, is) = 0.D0
        CALL POPREAL8(rt%rlbr(icv, 1, is), r8/8)
      ELSE
        CALL TRIMG_BWD(-20.0_R8, 20.0_R8, tt, ttb, rtb%rlbr(icv, 1, is))
        rtb%rlbr(icv, 1, is) = 0.D0
        CALL POPREAL8(rt%rlbr(icv, 1, is), r8/8)
      ...
      t0b = t0b - tempb
      CALL POPREAL8(t1, r8/8)
      fxneb = fxneb + (rtlbr(ite+1, ine+1, is)-rtlbr(ite+1, ine, is))*&
&       t1b + (rtlbr(ite, ine+1, is)-rtlbr(ite, ine, is))*t0b
      t0 = (1.0_R8-fxne)*rtlrd(ite, ine, is) + fxne*rtlrd(ite, ine+1, is&
&       )
      CALL POPREAL8(rt%rlrd(icv, 0, is), r8/8)
      fxteb = fxteb + (t1-t0)*rtb%rlrd(icv, 0, is)
      t0b = (1.0_R8-fxte)*rtb%rlrd(icv, 0, is)
      t1b = fxte*rtb%rlrd(icv, 0, is)
      rtb%rlrd(icv, 1, is) = rtb%rlrd(icv, 1, is) - rlte*rtb%rlrd(icv, 0&
&       , is)
      rlteb = rlteb - rt%rlrd(icv, 1, is)*rtb%rlrd(icv, 0, is)
      rtb%rlrd(icv, 0, is) = 0.D0
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL TRIMG_BWD(-20.0_R8, 20.0_R8, tt, ttb, rtb%rlrd(icv, 1, is))
        rtb%rlrd(icv, 1, is) = 0.D0
        CALL POPREAL8(rt%rlrd(icv, 1, is), r8/8)
      ELSE
        CALL TRIMG_BWD(-20.0_R8, 20.0_R8, tt, ttb, rtb%rlrd(icv, 1, is))
        rtb%rlrd(icv, 1, is) = 0.D0
        CALL POPREAL8(rt%rlrd(icv, 1, is), r8/8)
      ...
      t0b = t0b - tempb
      CALL POPREAL8(t1, r8/8)
      fxneb = fxneb + (rtlrd(ite+1, ine+1, is)-rtlrd(ite+1, ine, is))*&
&       t1b + (rtlrd(ite, ine+1, is)-rtlrd(ite, ine, is))*t0b
      t0 = (1.0_R8-fxne)*rtlqr(ite, ine, is) + fxne*rtlqr(ite, ine+1, is&
&       )
...
&              rt%rlra, 3)/8)
  CALL POPREAL8ARRAY(rt%rlbr, r8*SIZE(rt%rlbr, 1)*SIZE(rt%rlbr, 2)*SIZE(&
&              rt%rlbr, 3)/8)
  CALL POPREAL8ARRAY(rt%rlrd, r8*SIZE(rt%rlrd, 1)*SIZE(rt%rlrd, 2)*SIZE(&
&              rt%rlrd, 3)/8)
