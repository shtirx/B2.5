...
&       cvbb(icv, 3)
      IF (cfdna(7, is) .NE. 0.0_R8 .AND. ft_omp(ift) .NE. 0) THEN
        CALL PUSHREAL8(dna0(icv, is), r8/8)
        dna0(icv, is) = dna0(icv, is)*cfdna(7, is)*(geo%cvhy(icv)/geo%&
&         cvhy(omp(ft_omp(ift))))**2
        CALL PUSHCONTROL1B(1)
      ELSE
        CALL PUSHCONTROL1B(0)
      END IF
...
&       cfdpa(3, is)*6.25e-2_R8*pl%te(icv)/ev/geo%cvbb(icv, 3))
      IF (cfdpa(7, is) .NE. 0.0_R8 .AND. ft_omp(ift) .NE. 0) THEN
        CALL PUSHREAL8(dpa0(icv, is), r8/8)
        dpa0(icv, is) = dpa0(icv, is)*cfdpa(7, is)*(geo%cvhy(icv)/geo%&
&         cvhy(omp(ft_omp(ift))))**2
        CALL PUSHCONTROL1B(1)
      ELSE
        CALL PUSHCONTROL1B(0)
...
&       cvbb(icv, 3)
      IF (cfvla(7, is) .NE. 0.0_R8 .AND. ft_omp(ift) .NE. 0) THEN
        CALL PUSHREAL8(vla0(icv, 1, is), r8/8)
        vla0(icv, 1, is) = vla0(icv, 1, is)*cfvla(7, is)*geo%cvhy(icv)/&
&         geo%cvhy(omp(ft_omp(ift)))
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      END IF
...
&       )*df0 + cfvsa(3, is)*6.25e-2_R8*pl%te(icv)/ev/geo%cvbb(icv, 3)
      IF (cfvsa(7, is) .NE. 0.0_R8 .AND. ft_omp(ift) .NE. 0) THEN
        CALL PUSHREAL8(dfvsb, r8/8)
        dfvsb = dfvsb*cfvsa(7, is)*(geo%cvhy(icv)/geo%cvhy(omp(ft_omp(&
&         ift))))**2
        CALL PUSHCONTROL1B(1)
      ELSE
        CALL PUSHCONTROL1B(0)
...
&       )*df0 + cfhci(3, is)*6.25e-2_R8*pl%te(icv)/ev/geo%cvbb(icv, 3)
      IF (cfhci(7, is) .NE. 0.0_R8 .AND. ft_omp(ift) .NE. 0) THEN
        CALL PUSHREAL8(dfhcb, r8/8)
        dfhcb = dfhcb*cfhci(7, is)*(geo%cvhy(icv)/geo%cvhy(omp(ft_omp(&
&         ift))))**2
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
...
&     6.25e-2_R8*pl%te(icv)/ev/geo%cvbb(icv, 3)
    IF (cfhce(7) .NE. 0.0_R8 .AND. ft_omp(ift) .NE. 0) THEN
      CALL PUSHREAL8(dfhce, r8/8)
      dfhce = dfhce*cfhce(7)*(geo%cvhy(icv)/geo%cvhy(omp(ft_omp(ift))))&
&       **2
      CALL PUSHCONTROL1B(0)
    ELSE
      CALL PUSHCONTROL1B(1)
...
!WG_TODO     2    cfsig(4)*exp(-cfsig(5)*(lengthy(iCv)/lengthy(ix,ny))**2) !srv 26.11.04
    IF (cfsig(7) .NE. 0.0_R8 .AND. ft_omp(ift) .NE. 0) THEN
      CALL PUSHREAL8(dfsig, r8/8)
      dfsig = dfsig*cfsig(7)*(geo%cvhy(icv)/geo%cvhy(omp(ft_omp(ift))))&
&       **2
      CALL PUSHCONTROL1B(0)
    ELSE
      CALL PUSHCONTROL1B(1)
...
&     6.25e-2_R8*pl%te(icv)/ev/geo%cvbb(icv, 3)
    IF (cfalf(7) .NE. 0.0_R8 .AND. ft_omp(ift) .NE. 0) THEN
      CALL PUSHREAL8(dfalf, r8/8)
      dfalf = dfalf*cfalf(7)*(geo%cvhy(icv)/geo%cvhy(omp(ft_omp(ift))))&
&       **2
      CALL PUSHCONTROL1B(0)
    ELSE
      CALL PUSHCONTROL1B(1)
...
&                       hce_exb, hce_exbb, hci_exb, hci_exbb)
  ...
  IF (branch .EQ. 0) THEN
    ...
    CALL TRANSPORT_INPUT_B(ncv, ns, geo, geob, mpg, pl, plb, dv, dvb, rt&
&                    , rtb, hcib, hcibb, dna0, dna0b, dpa0, dpa0b, vla0&
&                    , vla0b, vsa0, vsa0b, hci0, hce0, hce0b, sig0, &
...
&       %ne(icv)*alf0b(icv)/(pl%te(icv)*2.0*temp2)
    ...
    IF (branch .EQ. 0) THEN
      ...
      tempb0 = geo%cvhy(icv)**2*dfalfb/geo%cvhy(omp(ft_omp(ift)))**2
      dfalfb = cfalf(7)*tempb0
      cfalfb(7) = cfalfb(7) + dfalf*tempb0
    END IF
    CALL POPREAL8(dfalf, r8/8)
    ...
    IF (branch .EQ. 0) THEN
      ...
      tempb0 = geo%cvhy(icv)**2*dfsigb/geo%cvhy(omp(ft_omp(ift)))**2
      dfsigb = cfsig(7)*tempb0
      cfsigb(7) = cfsigb(7) + dfsig*tempb0
    END IF
    CALL POPREAL8(dfsig, r8/8)
    ...
    IF (branch .EQ. 0) THEN
      ...
      tempb0 = geo%cvhy(icv)**2*dfhceb/geo%cvhy(omp(ft_omp(ift)))**2
      dfhceb = cfhce(7)*tempb0
      cfhceb(7) = cfhceb(7) + dfhce*tempb0
    END IF
    CALL POPREAL8(dfhce, r8/8)
...
&       tempb0
      ...
      IF (branch .EQ. 0) THEN
        ...
        tempb0 = geo%cvhy(icv)**2*dfhcbb/geo%cvhy(omp(ft_omp(ift)))**2
        dfhcbb = cfhci(7, is)*tempb0
        cfhcib(7, is) = cfhcib(7, is) + dfhcb*tempb0
      END IF
      CALL POPREAL8(dfhcb, r8/8)
      ...
      IF (branch .NE. 0) THEN
        ...
        tempb0 = geo%cvhy(icv)**2*dfvsbb/geo%cvhy(omp(ft_omp(ift)))**2
        dfvsbb = cfvsa(7, is)*tempb0
        cfvsab(7, is) = cfvsab(7, is) + dfvsb*tempb0
      END IF
      CALL POPREAL8(dfvsb, r8/8)
      ...
      IF (branch .EQ. 0) THEN
        ...
        tempb0 = geo%cvhy(icv)*vla0b(icv, 1, is)/geo%cvhy(omp(ft_omp(ift&
&         )))
        vla0b(icv, 1, is) = cfvla(7, is)*tempb0
        cfvlab(7, is) = cfvlab(7, is) + vla0(icv, 1, is)*tempb0
      END IF
      CALL POPREAL8(vla0(icv, 1, is), r8/8)
      ...
      IF (branch .NE. 0) THEN
        CALL POPREAL8(dpa0(icv, is), r8/8)
        tempb0 = geo%cvhy(icv)**2*dpa0b(icv, is)/geo%cvhy(omp(ft_omp(ift&
&         )))**2
        dpa0b(icv, is) = cfdpa(7, is)*tempb0
        cfdpab(7, is) = cfdpab(7, is) + dpa0(icv, is)*tempb0
      END IF
      CALL POPREAL8(dpa0(icv, is), r8/8)
...
&       tempb2
      ...
      IF (branch .NE. 0) THEN
        CALL POPREAL8(dna0(icv, is), r8/8)
        tempb0 = geo%cvhy(icv)**2*dna0b(icv, is)/geo%cvhy(omp(ft_omp(ift&
&         )))**2
        dna0b(icv, is) = cfdna(7, is)*tempb0
        cfdnab(7, is) = cfdnab(7, is) + dna0(icv, is)*tempb0
      END IF
      CALL POPREAL8(dna0(icv, is), r8/8)
