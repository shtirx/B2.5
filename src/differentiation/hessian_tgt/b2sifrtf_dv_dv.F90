...
!srv 13.01.17 { !srv 05.07.17
  ...
  arg1 = ncv*4
  CALL SFILL_NODIFF_NODIFF(arg1, 0.0_R8, smbtfial_nofl, 1)
!srv 13.01.17 }
  arg1 = ncv*4
...
& , nbdirsmax, ncv), cimp1dd(nbdirsmax0, nbdirsmax, ncv, 0:ns-1), &
& cimp2dd(nbdirsmax0, nbdirsmax, ncv, 0:ns-1), zeffdd(nbdirsmax0, &
& nbdirsmax, ncv), f_luc_sgdd(nbdirsmax0, nbdirsmax, nfc), alfx_cdd(&
& nbdirsmax0, nbdirsmax, ncv), sigx_cdd(nbdirsmax0, nbdirsmax, ncv)
!   ..output arguments (unspecified on entry)
!srv 13.01.17 05.07.17
  REAL(kind=r8) :: smbfria(ncv, 0:3), smbfrea(ncv, 0:3), smbtfia(ncv, 0:&
& 3), smbtfea(ncv, 0:3)
...
!     ..actual use of the Zhdanov closure
    DO icv=1,mpg%nci
      temp6 = rz2(icv, isb)/zetap(icv)
      DO nd0=1,nbdirs0
        tempd(nd0) = (rz2d0(nd0, icv, isb)-temp6*zetapd0(nd0, icv))/&
&         zetap(icv)
      END DO
      temp = temp6
      DO nd=1,nbdirs
        temp6 = rz2d(nd, icv, isb) - temp*zetapd(nd, icv)
        temp7 = na(icv, isb)/zetap(icv)
        DO nd0=1,nbdirs0
          t0dd(nd0, nd) = mp*(temp6*(nad0(nd0, icv, isb)-temp7*zetapd0(&
&           nd0, icv))/zetap(icv)+temp7*(rz2dd(nd0, nd, icv, isb)-zetapd&
&           (nd, icv)*tempd(nd0)-temp*zetapdd(nd0, nd, icv))+nad(nd, icv&
&           , isb)*tempd(nd0)+temp*nadd(nd0, nd, icv, isb))
        END DO
        t0d(nd) = mp*(temp7*temp6+temp*nad(nd, icv, isb))
      END DO
      DO nd0=1,nbdirs0
...
&                     nbdirs0)
          ...
          result1 = SQRT(arg10)
          temp = switch%b2sifr_phm0*mp*result1
          temp7 = kabvp(icv)/zetap(icv)
          DO nd0=1,nbdirs0
            temp0d(nd0) = (kabvpd0(nd0, icv)-temp7*zetapd0(nd0, icv))/&
&             zetap(icv)
            temp1d(nd0) = rz2(icv, is)*rz2d0(nd0, icv, isb) + rz2(icv, &
&             isb)*rz2d0(nd0, icv, is)
          END DO
          temp0 = temp7
          temp1 = rz2(icv, isb)*rz2(icv, is)
          DO nd=1,nbdirs
            temp7 = rz2(icv, is)*rz2d(nd, icv, isb) + rz2(icv, isb)*rz2d&
&             (nd, icv, is)
            temp6 = temp1/zetap(icv)
            temp8 = kabvpd(nd, icv) - temp0*zetapd(nd, icv)
            DO nd0=1,nbdirs0
              coefdd(nd0, nd) = temp*(temp7*temp0d(nd0)+temp0*(rz2d(nd, &
&               icv, isb)*rz2d0(nd0, icv, is)+rz2(icv, is)*rz2dd(nd0, nd&
&               , icv, isb)+rz2d(nd, icv, is)*rz2d0(nd0, icv, isb)+rz2(&
&               icv, isb)*rz2dd(nd0, nd, icv, is))+temp6*(kabvpdd(nd0, &
&               nd, icv)-zetapd(nd, icv)*temp0d(nd0)-temp0*zetapdd(nd0, &
&               nd, icv))+temp8*(temp1d(nd0)-temp6*zetapd0(nd0, icv))/&
&               zetap(icv))
...
&               (nd0, icv, isb) + na(icv, isb)*nadd(nd0, nd, icv, is)
            END DO
            coefd(nd) = temp*(temp0*temp7+temp8*temp6)
            t0d(nd) = na(icv, is)*nad(nd, icv, isb) + na(icv, isb)*nad(&
&             nd, icv, is)
          END DO
          DO nd0=1,nbdirs0
...
&                 ) + temp8*uad0(nd0, icv, is) + ua(icv, is)*(coefd(nd)*&
&                 t0d0(nd0)+t0*coefdd(nd0, nd)+t0d(nd)*coefd0(nd0)+coef*&
&                 t0dd(nd0, nd)) + uad(nd, icv, is)*(t0*coefd0(nd0)+coef&
&                 *t0d0(nd0)) + coef*t0*uadd(nd0, nd, icv, is)
...
&                     result1dd, nbdirs, nbdirs0)
            ...
            DO nd0=1,nbdirs0
              temp1d(nd0) = (t0*coefd0(nd0)+coef*t0d0(nd0)-temp8*&
&               result1d0(nd0))/result1
            END DO
            temp1 = temp8
            DO nd=1,nbdirs
              temp8 = t0*coefd(nd) + coef*t0d(nd)
...
&                 ) - (coefd(nd)*t0d0(nd0)+t0*coefdd(nd0, nd)+t0d(nd)*&
&                 coefd0(nd0)+coef*t0dd(nd0, nd)-result1d(nd)*temp1d(nd0&
&                 )-temp1*result1dd(nd0, nd)-temp8*result1d0(nd0))/&
&                 result1
              END DO
...
&                 ) + temp7*(t0*coefd0(nd0)+coef*t0d0(nd0)-temp6*&
&                 result1d0(nd0))/result1 + temp6*(uadd(nd0, nd, icv, is&
&                 )-result1d(nd)*temp1d(nd0)-temp1*result1dd(nd0, nd))
              END DO
              smbfriald(nd, icv, 2) = smbfriald(nd, icv, 2) + temp1*&
&               temp8 + temp6*temp7
            END DO
            CALL ROXA_DV_DV(icv, isb, result1, result1d0, result1d, &
&                     result1dd, nbdirs, nbdirs0)
...
&               )+temp5*(rz2d(nd, icv, isb)*calfabd0(nd0)+calfab*rz2dd(&
&               nd0, nd, icv, isb)+calfabd(nd)*rz2d0(nd0, icv, isb)+rz2(&
&               icv, isb)*calfabdd(nd0, nd))+temp9*(rz2dd(nd0, nd, icv, &
&               is)-qe*(zetapd(nd, icv)*temp5d(nd0)+temp5*zetapdd(nd0, &
&               nd, icv)))+temp8*(temp4d(nd0)-temp9*qe*zetapd0(nd0, icv)&
&               )/(qe*zetap(icv)))
            END DO
...
&                 +result1d(nd)*kabvpd0(nd0, icv)+kabvp(icv)*result1dd(&
&                 nd0, nd))+temp9*(result1*kabvpd0(nd0, icv)+kabvp(icv)*&
...
&               gtid(nd, icv)*nad0(nd0, icv, isb)+na(icv, isb)*gtidd(nd0&
&               , nd, icv)) + temp9*(nadd(nd0, nd, icv, is)-temp8*temp4d&
&               (nd0)-temp4*(nadd(nd0, nd, icv, isb)+nadd(nd0, nd, icv, &
&               is))) + temp7*(temp3d(nd0)-temp9*temp5d(nd0))/temp5
            END DO
...
&               coef*fllim_al_abdd(nd0, nd)+coefd(nd)*fllim_al_abd0(nd0)&
&               +fllim_al_ab*coefdd(nd0, nd)) + t0d(nd)*(coef*&
...
&       .NOT.is_neutral(b))) THEN
      DO nd=1,nbdirs
        DO nd0=nd,nbdirs0
          fkabvpdd(nd0, nd) = cimp1dd(nd0, nd, icv, b)
        END DO
        fkabvpd(nd) = cimp1d(nd, icv, b)
      END DO
      DO nd0=1,nbdirs0
        fkabvpd0(nd0) = cimp1d0(nd0, icv, b)
      END DO
      fkabvp = cimp1(icv, b)
    ELSE IF (b .EQ. ismain .AND. a .NE. b .AND. (.NOT.is_neutral(a)) &
&       .AND. (.NOT.is_neutral(b))) THEN
      DO nd=1,nbdirs
        DO nd0=nd,nbdirs0
          fkabvpdd(nd0, nd) = cimp1dd(nd0, nd, icv, a)
        END DO
        fkabvpd(nd) = cimp1d(nd, icv, a)
      END DO
      DO nd0=1,nbdirs0
        fkabvpd0(nd0) = cimp1d0(nd0, icv, a)
      END DO
      fkabvp = cimp1(icv, a)
    ELSE IF (a .NE. b .AND. a .NE. ismain .AND. b .NE. ismain .AND. (&
&       .NOT.is_neutral(a)) .AND. (.NOT.is_neutral(b))) THEN
...
&       .NOT.is_neutral(b))) THEN
      DO nd=1,nbdirs
        DO nd0=nd,nbdirs0
          fkabtfdd(nd0, nd) = cimp2dd(nd0, nd, icv, a)
        END DO
        fkabtfd(nd) = cimp2d(nd, icv, a)
      END DO
      DO nd0=1,nbdirs0
        fkabtfd0(nd0) = cimp2d0(nd0, icv, a)
      END DO
      fkabtf = cimp2(icv, a)
    ELSE IF (a .NE. ismain .AND. b .NE. ismain .AND. (.NOT.is_neutral(a)&
&       ) .AND. (.NOT.is_neutral(b))) THEN
