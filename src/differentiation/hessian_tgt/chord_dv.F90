...
!
!
SUBROUTINE CHORD_COMPILE_NODIFF0(npoints, mpg, geo, xp1, yp1, zp1, xp2, &
...
!
  ...
  LOGICAL :: vector
!
  REAL(kind=r8) :: CHORD_VALUE_NODIFF0
  INTEGER :: i, icv
...
!
  DO i=1,npoints
    ...
    y = yp1*(1.0-f) + yp2*f
    z = zp1*(1.0-f) + zp2*f
    CALL CHORD_FIND_NODIFF0(mpg, geo, x, y, z, found, icv)
    IF (found) THEN
      result1 = CHORD_VALUE_NODIFF0(mpg%ncv, tt, x, y, z, dxp, dyp, dzp&
&       , vector)
...
!
  CALL SUBEND()
  RETURN
END SUBROUTINE CHORD_COMPILE_NODIFF0
...
!
!
FUNCTION CHORD_VALUE_NODIFF0(ncv, tt, xp, yp, zp, dxp, dyp, dzp, vector)&
&RESULT (chord_value)
!
  USE B2MOD_TYPES
...
!       write(*,*) 'dl.bl = ',dotprod
    ...
    RETURN
  END IF
END FUNCTION CHORD_VALUE_NODIFF0

!
SUBROUTINE CHORD_FIND_NODIFF0(mpg, geo, xp, yp, zp, found, cvpos)
!
  USE B2MOD_TYPES
  USE B2US_MAP_DIFFV_DIFFV
  USE B2US_GEO_DIFFV_DIFFV
...
& ) THEN
  ...
  ELSE
    ...
    IF (jpos .NE. 0) THEN
      IF (INSIDE_CELL_NODIFF0(mpg, geo, r, z, icvsv)) GOTO 2100
    END IF
! next try the chain
    DO WHILE (jpos .NE. 0)
...
END SUBROUTINE CHORD_FIND_NODIFF0

!
FUNCTION INSIDE_CELL_NODIFF0(mpg, geo, r, z, icv) RESULT (inside_cell)
  USE B2MOD_TYPES
  USE B2US_MAP_DIFFV_DIFFV
  USE B2US_GEO_DIFFV_DIFFV
  USE B2MOD_CONSTANTS
...
!
    RETURN
  END IF
END FUNCTION INSIDE_CELL_NODIFF0
