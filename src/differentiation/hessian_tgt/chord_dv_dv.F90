...
!
!
SUBROUTINE CHORD_COMPILE_NODIFF_NODIFF(npoints, mpg, geo, xp1, yp1, zp1&
& , xp2, yp2, zp2, vector, indchd, indcv, indscl)
!
  USE B2MOD_TYPES
...
!
  ...
  LOGICAL :: vector
!
  REAL(kind=r8) :: CHORD_VALUE_NODIFF_NODIFF
  INTEGER :: i, icv
...
!
  DO i=1,npoints
    ...
    y = yp1*(1.0-f) + yp2*f
    z = zp1*(1.0-f) + zp2*f
    CALL CHORD_FIND_NODIFF_NODIFF(mpg, geo, x, y, z, found, icv)
    IF (found) THEN
      result1 = CHORD_VALUE_NODIFF_NODIFF(mpg%ncv, tt, x, y, z, dxp, dyp&
&       , dzp, vector)
...
!
  CALL SUBEND()
  RETURN
END SUBROUTINE CHORD_COMPILE_NODIFF_NODIFF
...
!
!
FUNCTION CHORD_VALUE_NODIFF_NODIFF(ncv, tt, xp, yp, zp, dxp, dyp, dzp, &
...
!
  ...
  EXTERNAL CHORD_FIND_NODIFF_NODIFF, XERTST
  REAL(kind=r8) :: abs0
  REAL(kind=r8) :: arg1
  REAL(kind=r8) :: result1
...
!       write(*,*) 'dl.bl = ',dotprod
    ...
    RETURN
  END IF
END FUNCTION CHORD_VALUE_NODIFF_NODIFF

!
SUBROUTINE CHORD_FIND_NODIFF_NODIFF(mpg, geo, xp, yp, zp, found, cvpos)
!
  USE B2MOD_TYPES
  USE B2US_MAP_DIFFV_DIFFV
  USE B2US_GEO_DIFFV_DIFFV
...
!     for adjoint AD to avoid side-effect variables
  USE B2MOD_AD_DIFFV_DIFFV, ONLY : covered, rmin, rmax, zmin, zmax, &
& first, cvcov, icov, icvsv, lngind, lngcov
  USE B2MOD_DIFFSIZES
  IMPLICIT NONE
!
...
!
  ...
  INTEGER :: cvpos
!
  LOGICAL :: INSIDE_CELL_NODIFF_NODIFF
  INTEGER :: icnt1, icnt2
...
& ) THEN
  ...
  ELSE
    ...
    IF (jpos .NE. 0) THEN
      result11 = INSIDE_CELL_NODIFF_NODIFF(mpg, geo, r, z, icvsv)
      IF (result11) GOTO 2100
    END IF
! next try the chain
    DO WHILE (jpos .NE. 0)
...
END SUBROUTINE CHORD_FIND_NODIFF_NODIFF

!
FUNCTION INSIDE_CELL_NODIFF_NODIFF(mpg, geo, r, z, icv) RESULT (&
&inside_cell)
  USE B2MOD_TYPES
  USE B2US_MAP_DIFFV_DIFFV
  USE B2US_GEO_DIFFV_DIFFV
...
!
    RETURN
  END IF
END FUNCTION INSIDE_CELL_NODIFF_NODIFF
