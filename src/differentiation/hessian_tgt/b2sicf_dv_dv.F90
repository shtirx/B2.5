...
!
!  Differentiation of b2sicf_dv in forward (tangent) mode (with options multiDirectional context noISIZE r8):
!   variations   of useful results: ctcfd wrk0 ctcf smbcfd smbcf
!                wrk0d
!   with respect to varying inputs: nb ub ctcfd wrk0 nbd ubd smbcfd
!                wrk0d
...
!srv 28.01.02
  ...
  REAL(kind=r8) :: nbd0(nbdirsmax0, ncv), ubd0(nbdirsmax0, ncv)
  REAL(kind=r8) :: nbd(nbdirsmax, ncv), ubd(nbdirsmax, ncv)
  REAL(kind=r8) :: nbdd(nbdirsmax0, nbdirsmax, ncv), ubdd(nbdirsmax0, &
& nbdirsmax, ncv)
...
& )
  ...
  REAL(kind=r8) :: smbcfdd(nbdirsmax0, nbdirsmax, ncv, 0:3), ctcfdd(&
...
!srv 06.04.07
  ...
  IF (switch%b2sicf_style .EQ. 0) THEN
    CALL GRADC_P_NODIFF_NODIFF(ncv, nfc, nvx, 1, geo, mpg, geo%cvhz, geo&
&                        %vxhz, wrk0)
    DO nd=1,nbdirsmax
      DO nd0=1,nbdirs0
        ctcfdd(nd0, nd, :) = 0.0_8
...
&       b2sicf_phm0*mb
      DO nd0=1,nbdirs0
        temp0d(nd0) = nbd0(nd0, icv)/geo%cvbb(icv, 3)
      END DO
      temp0 = nb(icv)/geo%cvbb(icv, 3)
      DO nd=1,nbdirs
        temp1 = 2*temp0*ub(icv)
        temp2 = nbd(nd, icv)/geo%cvbb(icv, 3)
        DO nd0=1,nbdirs0
          smbcfdd(nd0, nd, icv, 0) = temp*(ubd(nd, icv)*(ub(icv)*2*&
&           temp0d(nd0)+2*temp0*ubd0(nd0, icv))+temp1*ubdd(nd0, nd, icv)&
&           +temp2*2*ub(icv)*ubd0(nd0, icv)+ub(icv)**2*nbdd(nd0, nd, icv&
&           )/geo%cvbb(icv, 3))
        END DO
        smbcfd(nd, icv, 0) = temp*(temp1*ubd(nd, icv)+ub(icv)*ub(icv)*&
&         temp2)
      END DO
      DO nd0=1,nbdirs0
        smbcfd0(nd0, icv, 0) = temp*(temp0*2*ub(icv)*ubd0(nd0, icv)+ub(&
&         icv)**2*temp0d(nd0))
      END DO
...
&       switch%b2sicf_phm0*geo%cvbb(icv, 0)
      DO nd=1,nbdirs
        DO nd0=nd,nbdirs0
          ctcfdd(nd0, nd, icv) = temp0*(ubd(nd, icv)*nbd0(nd0, icv)+nb(&
&           icv)*ubdd(nd0, nd, icv)+nbd(nd, icv)*ubd0(nd0, icv)+ub(icv)*&
&           nbdd(nd0, nd, icv))/geo%cvbb(icv, 3)
        END DO
        ctcfd(nd, icv) = temp0*(nb(icv)*ubd(nd, icv)+ub(icv)*nbd(nd, icv&
&         ))/geo%cvbb(icv, 3)
      END DO
      DO nd0=1,nbdirs0
        ctcfd0(nd0, icv) = temp0*(nb(icv)*ubd0(nd0, icv)+ub(icv)*nbd0(&
&         nd0, icv))/geo%cvbb(icv, 3)
      END DO
...
! ..return
  ...
  CALL SUBEND()
  RETURN
END SUBROUTINE B2SICF_DV_NODIFF
...
! ..return
  ...
  CALL SUBEND()
  RETURN
END SUBROUTINE B2SICF_NODIFF_NODIFF
