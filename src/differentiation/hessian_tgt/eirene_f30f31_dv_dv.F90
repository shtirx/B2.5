...
!
!
SUBROUTINE COPY_BACKGROUND_NODIFF_NODIFF(ncv, nfc, ns, switch, mpg, gm, &
& pl, dv, st_ext)
  USE B2MOD_TYPES
  ...
  USE B2MOD_DIAG_DIFFV_DIFFV, ONLY : fluids_list
  USE B2MOD_SUBSYS
  USE B2MOD_BRAEIR_DIFFV_DIFFV
  USE B2MOD_NEUTRALS_NAMELIST_DIFFV_DIFFV, ONLY : rcfe, rcfi, mol
  USE B2MOD_RECYCLE_DIFFV_DIFFV, ONLY : fna_mol
...
&                        , 0, is), vaecrbc(:, 0, is))
    CALL INTCELL_NODIFF_NODIFF(nfc, ncv, mpg, mpg%intcellp, dv%wadia(:, &
&                        0, is), wadiac(:, 0, is))
    CALL INTCELL_NODIFF_NODIFF(nfc, ncv, mpg, mpg%intcellp, dv%vadia(:, &
&                        0, is), vadiac(:, 0, is))
    CALL INTCELL_NODIFF_NODIFF(nfc, ncv, mpg, mpg%intcellr, dv%vaecrb(:&
&                        , 1, is), vaecrbc(:, 1, is))
    CALL INTCELL_NODIFF_NODIFF(nfc, ncv, mpg, mpg%intcellr, dv%wadia(:, &
&                        1, is), wadiac(:, 1, is))
    CALL INTCELL_NODIFF_NODIFF(nfc, ncv, mpg, mpg%intcellr, dv%vadia(:, &
&                        1, is), vadiac(:, 1, is))
  END DO
...
&       nfc, 0, 0:ns-1)
    END IF
    arg111 = ns - 1
    CALL INTCELL1_NODIFF_NODIFF(nfc, ncv, arg111, mpg, mpg%intcellp, &
&                         fna_tmp, uuc)
  END IF
!
  DO is=0,ns-1
...
&       is)
  ...
  END DO
  arg111 = ns - 1
  CALL INTCELL1_NODIFF_NODIFF(nfc, ncv, arg111, mpg, mpg%intcellr, &
&                       fna_tmp, vvc)
!
  ...
  IF (switch%spatial_hybrid .NE. 0) THEN
    DO is=0,ns-1
      IF (is_neutral(is)) THEN
        fna_tmp(1:nfc, is) = dv%fna(1:nfc, 1, is)/nafsqalf(1:nfc, 1, is)
        CALL INTCELL_NODIFF_NODIFF(nfc, ncv, mpg, mpg%intcellr, fna_tmp(&
&                            1:nfc, is), vvc(1:nfc, is))
      END IF
    END DO
  END IF
!
