...
!                *(srd.skt) *(srd.szt) *(srd.smo) *(srd.sna) *(srd.shedt)
!                *(srd.sktdt) *(srd.sztdt) *(srd.shidt) *(srd.shndt)
!                *(srd.schdt) *(srd.smodt) *(srd.snadt) ti0d ua0d
!                tn0d te0 ne0d kinrgy0d na0 zt0d nn0 kt0 *(sr.sch)
...
!                srd.shedt:in srd.sktdt:in srd.sztdt:in srd.shidt:in
!                srd.shndt:in srd.schdt:in srd.smodt:in srd.snadt:in
!                sr.sch:in sr.she:in sr.shi:in sr.sne:in sr.shn:in
!                sr.skt:in sr.szt:in sr.smo:in sr.sna:in sr.shedt:in
...
& kinrgy0d, kinrgy0dd, kt0, kt0d0, kt0d, kt0dd, zt0, zt0d0, zt0d, zt0dd&
& , na, nad, ua, te, ted, ti, tid, tn, tnd, ne, ned, ni, nid, nn, nnd, &
& kinrgy, kinrgyd, kt, ktd, zt, ztd, sr, srd0, srd, srdd, lout, nbdirs, &
& nbdirs0)
  USE B2MOD_TYPES
...
& nbdirsmax0, ncv), zt0d0(nbdirsmax0, ncv)
...
!fc 11.03.24
      ...
      DO nd=1,nbdirs
        DO nd0=nd,nbdirs0
          srdd%snadt(nd0, nd, icv, 0, is) = t0*na0dd(nd0, nd, icv, is)
          srdd%snadt(nd0, nd, icv, 1, is) = 0.D0
        END DO
        srd%snadt(nd, icv, 0, is) = t0*na0d(nd, icv, is)
        srd%snadt(nd, icv, 1, is) = 0.d0
      END DO
      DO nd0=1,nbdirs0
        srd0%snadt(nd0, icv, 0, is) = t0*na0d0(nd0, icv, is)
        srd0%snadt(nd0, icv, 1, is) = 0.D0
      END DO
...
&           , nd, icv, is))
          ...
          srdd%smodt(nd0, nd, icv, 3, is) = 0.D0
        END DO
...
! when using a pure ion energy equation (tn_style=1)
      temp1 = ni0(icv, 1)/ni0(icv, 0)
      DO nd0=1,nbdirs0
        tempd(nd0) = (ni0d0(nd0, icv, 1)-temp1*ni0d0(nd0, icv, 0))/ni0(&
&         icv, 0)
      END DO
      temp = temp1
      DO nd=1,nbdirs
        temp1 = (ni0d(nd, icv, 1)-temp*ni0d(nd, icv, 0))/ni0(icv, 0)
        DO nd0=1,nbdirs0
!
          srdd%shidt(nd0, nd, icv, 0) = t0*1.5_R8*(ni0d(nd, icv, 1)*&
&           ti0d0(nd0, icv)+ti0(icv)*ni0dd(nd0, nd, icv, 1)+ti0d(nd, icv&
&           )*ni0d0(nd0, icv, 1)+ni0(icv, 1)*ti0dd(nd0, nd, icv))
          srdd%shidt(nd0, nd, icv, 1) = 0.D0
          srdd%shidt(nd0, nd, icv, 2) = 0.D0
          srdd%shidt(nd0, nd, icv, 3) = -(t0*1.5_R8*(ni0dd(nd0, nd, icv&
&           , 1)-ni0d(nd, icv, 0)*tempd(nd0)-temp*ni0dd(nd0, nd, icv, 0)&
&           -temp1*ni0d0(nd0, icv, 0))/ni0(icv, 0))
        END DO
        srd%shidt(nd, icv, 0) = t0*1.5_R8*(ti0(icv)*ni0d(nd, icv, 1)+ni0&
&         (icv, 1)*ti0d(nd, icv))
        srd%shidt(nd, icv, 1) = 0.d0
        srd%shidt(nd, icv, 2) = 0.d0
        srd%shidt(nd, icv, 3) = -(t0*1.5_R8*temp1)
      END DO
      DO nd0=1,nbdirs0
        srd0%shidt(nd0, icv, 0) = t0*1.5_R8*(ti0(icv)*ni0d0(nd0, icv, 1)&
&         +ni0(icv, 1)*ti0d0(nd0, icv))
        srd0%shidt(nd0, icv, 1) = 0.D0
        srd0%shidt(nd0, icv, 2) = 0.D0
        srd0%shidt(nd0, icv, 3) = -(t0*1.5_R8*tempd(nd0))
      END DO
