...
& ), fhi_visq(:, :), fhi_anml(:, :), fhi_kevis(:, :), fhi_vzh(:, :)
!     Source
...
& fhi_kevis_av(:, :), fhi_vzh_av(:, :), b2stbr_phys_shi_bal_av(:), &
...
!
!djm Jan2017
  SUBROUTINE UPDATE_BALANCE_EIRENE_SOURCES_PARTICLES(ncv, isb, nb)
  USE B2MOD_DIFFSIZES
    IMPLICIT NONE
!   ..input arguments (unchanged on exit)
    INTEGER :: ncv, isb
...
&         , isb) + eirene_mc_core_sna0to1(icv, 1, isb)*nb(icv)
    ...
    END DO
    RETURN
  END SUBROUTINE UPDATE_BALANCE_EIRENE_SOURCES_PARTICLES
...
!
  SUBROUTINE UPDATE_BALANCE_EIRENE_SOURCES_MOMENTUM(ncv, isb, ub, rob)
  USE B2MOD_DIFFSIZES
    IMPLICIT NONE
!   ..input arguments (unchanged on exit)
    INTEGER :: ncv, isb
...
&           eirene_mc_mppl_smo0to3(icv, 3, isb, istrai)*rob(icv)*ub(icv)
    ...
    END DO
    RETURN
  END SUBROUTINE UPDATE_BALANCE_EIRENE_SOURCES_MOMENTUM
...
!
  SUBROUTINE UPDATE_BALANCE_EIRENE_SOURCES_HEAT(ncv, ne, ni, te, ti)
  USE B2MOD_DIFFSIZES
    IMPLICIT NONE
!   ..input arguments (unchanged on exit)
    INTEGER :: ncv
...
&           eirene_mc_eppl_shi0to3(icv, 3, istrai)*ni(icv)*ti(icv)
    ...
    END DO
    RETURN
  END SUBROUTINE UPDATE_BALANCE_EIRENE_SOURCES_HEAT
...
!   ..Arrays for ion heat balance:
    ...
    fhi_vzh_av = avmlt*fhi_vzh_av + curmlt*fhi_vzh
...
!
  SUBROUTINE WRITE_BALANCE(ncv, nfc, nvx, nsd, geo, mpg)
    ...
    USE B2MOD_B2CMPA, ONLY : zamin, zamax
...
!   ..procedures
    INTRINSIC SIZE
    EXTERNAL IPGETI, GET_JSEP, IPGETR, SPECIES, CHECK_CDF_STATUS
    INTRINSIC ALLOCATED
    INTRINSIC NINT
...
&                   2)
    ...
    CALL WRITE_CDF_DOUBLE(ncid, 'fhi_vzh', fhi_vzh_av, vdims, 2)
!     Sources
    vdims(1) = ncvpid
